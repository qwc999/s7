...
for i, (label, group) in enumerate(results.items(), 1):
    ...
    acnum, pos = label.split('_')[0], int(label.split('_')[2])
    data_group = df[(df[acnum_column_name] == acnum) & (df[pos_column_name] == pos)]
    data_group_last_25 = data_group.iloc[int(len(data_group) * 0.75):]

    # Убедиться, что размеры совпадают
    data_group_last_25 = data_group_last_25.iloc[:len(predictions)]

    plt.subplot(2, 2, i)
    plt.plot(data_group_last_25['reportts'], data_group_last_25[egtm_column_name], label=f'{label} True')
    plt.plot(data_group_last_25['reportts'], predictions, label=f'{label} Predicted', color='orange')
