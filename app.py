...
async def plot_regression():
    ...
    return HTMLResponse(content=generate_page(plot_div))


@app.get("/regression", response_class=HTMLResponse)
async def plot_regression(engine_type: str, side: Literal["left", "right"]):
    ...
    graphs = []
    for i, (label, group) in enumerate(results.items(), 1):
        if label != f"{engine_type}_pos_{side}":
            continue
        rmse, mae, indices, predictions = group
        data_group = df[(df[acnum_column_name] == label.split('_')[0]) & (df[pos_column_name] == int(label.split('_')[2]))]
    ...
    if not graphs:
        raise HTTPException(404, "Not found this engine")

    fig = make_subplots(rows=2, cols=2, subplot_titles=[graphs[0][0]])
    fig.add_trace(
        graphs[0][1],
        row=1, 
        col=1
